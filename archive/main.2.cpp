...
void save(String _ssid, String _pass) {
  EEPROM.begin(512);
  put(_ssid, ssid_addr);
  put(_pass, pswd_addr);
  EEPROM.commit();
...
void longPress(function<void()> func) {
  if (!digitalRead(BUTTON)) {
    ...
    if (press_mem < millis()) {
      Serial.println("button press action");
      func();
      press_mem = 0;
      return;
    }
      ...
      while (client.connected()) {
        ...
        if (method == "GET" && src == "/") {
          ...
          if (true) {
            ...
            String _ssid = buf.substring(buf.indexOf('=') + 1, and1);
            String _pass = buf.substring(buf.indexOf('=', and1) + 1);
            save(_ssid, _pass);
